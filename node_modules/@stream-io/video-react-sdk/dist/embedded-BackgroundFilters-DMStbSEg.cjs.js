...
const useTrackFramesPerSecond = (call) => {
    ...
    react.useEffect(() => {
        ...
        const intervalId = setInterval(() => {
            const track = rootMediaStream?.getVideoTracks()[0];
            ...
            if (previousSnapshot) {
                const elapsedSec = (currentSnapshot.capturedAt - previousSnapshot.capturedAt) / 1000;
                const delivered = currentSnapshot.deliveredFrames - previousSnapshot.deliveredFrames;
...
const BackgroundFiltersProvider = (props) => {
    const { ContextProvider, children, backgroundImages = EMPTY_BACKGROUND_IMAGES, backgroundFilter: bgFilterFromProps = undefined, backgroundImage: bgImageFromProps = undefined, backgroundBlurLevel: bgBlurLevelFromProps = undefined, tfFilePath, modelFilePath, useLegacyFilter, basePath, onError, forceSafariSupport, forceMobileSupport, segmentationOptions, } = props;
    ...
    const fpsRatioEmaRef = react.useRef(undefined);
...
const useRenderer = (api, tfLite, call, engine) => {
    ...
    const processorRef = react.useRef(undefined);
    ...
    const startMediaPipe = react.useCallback((ms, onError, onStats) => {
        let outputStream;
        let processor;
        const output = new Promise((resolve, reject) => {
            ...
            processor = new videoFiltersWeb.VirtualBackground(track, { ...options, basePath, modelPath: modelFilePath }, { onError, onStats });
    ...
    const startTf = react.useCallback((ms, onError) => {
        let outputStream;
        let renderer;
        const output = new Promise((resolve, reject) => {
            if (!backgroundFilter) {
    ...
    react.useEffect(() => {
        ...
        processorRef.current
            ...
            .catch((err) => console.warn(`[filters] Can't apply the background options`, err));
    }, [
        backgroundFilter,
        backgroundBlurLevel,
        backgroundImage,
...
//# sourceMappingURL=embedded-BackgroundFilters-DMStbSEg.cjs.js.map
