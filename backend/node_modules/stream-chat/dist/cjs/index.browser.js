...
__export(index_exports, {
  ...
  createSendWithPendingUploadsAttachmentsMiddleware: () => createSendWithPendingUploadsAttachmentsMiddleware,
  ...
  isUploadCancellation: () => isUploadCancellation,
  ...
  withoutConcurrency: () => withoutConcurrency
});
...
var isRemoteUrl = (url) => !!url && /^https?:\/\//i.test(url.trim());
var sanitizeOutgoingAttachments = ({
  client,
  message
}) => {
  ...
  const unresolved = [];
  let changed = false;
  const sanitized = attachments.reduce((acc, attachment) => {
    ...
    changed = true;
    const hasRemoteSource = isRemoteUrl(rest.asset_url) || isRemoteUrl(rest.image_url) || isRemoteUrl(rest.og_scrape_url) || isRemoteUrl(rest.title_link);
    if (hasRemoteSource) {
      acc.push(rest);
    } else {
...
var PENDING_UPLOAD_STATES = [
  "pending",
  "uploading"
];
var isPendingUpload = (attachment) => isLocalUploadAttachment(attachment) && PENDING_UPLOAD_STATES.includes(attachment.localMetadata.uploadState);
var isFinishedUpload = (attachment) => isLocalUploadAttachment(attachment) && attachment.localMetadata.uploadState === "finished";
...
var createPostUploadAttachmentEnrichmentMiddleware = (composer) => ({
  ...
  handlers: {
    postProcess: ({
      ...
      const attachmentIsStillInComposer = !composer || !!composer.attachmentManager.attachmentsById[id];
...
var isUploadCancellation = (error) => import_axios.default.isCancel(error) || error instanceof Error && error.name === "AbortError";
var initState = () => ({ uploads: {} });
        ...
        try {
          ...
          const onProgress = trackProgress ? (progress) => {
            this.updateUpload({
              ...
              ...typeof progress === "number" ? { uploadConfirmationPending: progress >= 100 } : {}
            });
          } : void 0;
...
var _AttachmentManager = class _AttachmentManager {
  constructor({ composer, message }) {
    ...
    this.initState = ({ message } = {}) => {
      this.state.next(initState2({ message }));
    };
    this.getSnapshot = () => {
     ...
     */
    this.revokePreviewUri = (attachment) => {
    ...
    this.removeAttachments = (localAttachmentIds) => {
      ...
      const removedAttachments = this.attachments.filter(
    ...
    this.state = new StateStore(initState2({ message }));
...
var composeWithPendingUploads = ({
  composer,
  state
}) => {
  const composerIsKeptAsDraft = !!composer.pollId;
  const relevantAttachments = composer.attachmentManager.attachments.filter(
    (attachment) => isFinishedUpload(attachment) || !composerIsKeptAsDraft && isPendingUpload(attachment)
  );
  const localAttachments = (state.localMessage.attachments ?? []).concat(
    relevantAttachments.map(
      (attachment) => isPendingUpload(attachment) ? attachment : localAttachmentToAttachment(attachment)
    )
  );
...
var createSendWithPendingUploadsAttachmentsMiddleware = (composer) => ({
...
var BaseSearchSourceBase = class {
  constructor(options) {
    this.setDebounceOptions = (options = {}) => {
      ...
      this.searchDebounced = debounce(
        ...
        (searchString) => this.getDebounceMs(searchString)
      );
    };
    /**
    ...
    this.getDebounceMs = (searchString) => {
      ...
      return length <= this.shortQueryMaxLength ? this.shortQueryDebounceMs : this.longQueryDebounceMs;
    };
    ...
    this.canExecuteQuery = (newSearchString) => {
      ...
      return !!(this.isActive && this.canDispatchQuery(hasNewSearchQuery) && (this.allowEmptySearchString || searchString));
    };
   ...
   */
  canDispatchQuery(hasNewSearchQuery) {
...
var ChannelMemberSearchSource = class extends BaseSearchSource {
  ...
  async query(searchQuery, queryOptions = {}) {
...
var MessageComposerMiddlewareExecutor = class extends MiddlewareExecutor {
   ...
   */
  get installedMiddleware() {
    return super.installedMiddleware;
  }
};
...
var MentionsSearchSource = class extends BaseSearchSource {
  constructor(channel, options) {
    ...
    this.getRoleMentionSuggestions = async (query, queryOptions = {}) => {
      if (!this.isMentionTypeAllowed("role")) return [];
      if (!query) return [];
      const { roles } = await this.client.searchRoles({ query }, queryOptions);
    ...
    this.queryUsers = async (searchQuery, offset = 0, queryOptions = {}) => {
      const { filters, sort, options } = this.prepareQueryUsersParams(searchQuery, offset);
      const { users } = await this.client.queryUsers(filters, sort, options, queryOptions);
      return users;
    };
    this.queryMembers = async (searchQuery, offset = 0, queryOptions = {}) => {
    ...
    this.getUserSuggestionsPage = async (searchQuery, userOffset = 0, queryOptions = {}) => {
      ...
      if (this.config.mentionAllAppUsers) {
        users = await this.queryUsers(searchQuery, userOffset, queryOptions);
        ...
        users = await this.queryMembers(searchQuery, userOffset, queryOptions);
      }
    ...
    this.getUserGroupSuggestionsPage = async (searchQuery, cursor, queryOptions = {}) => {
      ...
      const { user_groups } = await this.client.searchUserGroups(options, queryOptions);
      return {
  ...
  async query(searchQuery, queryOptions = {}) {
    ...
    const [userResultsState, userGroupResultsState, roleSuggestionsState] = await Promise.allSettled([
      this.getUserSuggestionsPage(searchQuery, userOffset, queryOptions),
      this.getUserGroupSuggestionsPage(
        searchQuery,
        previousUserGroupCursor,
        queryOptions
      ),
      isFirstPage ? this.getRoleMentionSuggestions(searchQuery, queryOptions) : Promise.resolve([])
    ]);
    if (queryOptions.signal?.aborted) return { items: [] };
...
var _LinkPreviewsManager = class _LinkPreviewsManager {
  constructor({ composer, message }) {
    ...
    this.initState = ({ message } = {}) => {
      ...
      this.state.next(initState5({ message: this.enabled ? message : void 0 }));
    };
    this.getSnapshot = () => this.state.getLatestValue();
    this.restoreSnapshot = (snapshot) => {
      this.state.next(snapshot);
    ...
    this.composer = composer;
    this.state = new StateStore(
      initState5({ message: this.enabled ? message : void 0 })
    );
...
var LocationComposer = class {
  constructor({ composer, message }) {
    this.initState = ({ message } = {}) => {
      this.state.next(initState6({ message }));
    };
    this.getSnapshot = () => this.state.getLatestValue();
    this.restoreSnapshot = (snapshot) => {
      this.state.next(snapshot);
    ...
    this.composer = composer;
    this.state = new StateStore(initState6({ message }));
...
var _MessageComposer = class _MessageComposer extends WithSubscriptions {
  ...
  constructor({
    ...
    this.initState = ({
      ...
      this.state.next(initState7(composition));
    };
    ...
    this.state = new StateStore(initState7(composition));
  ...
  get hasSendableData() {
    ...
    if (this.allowsPendingUploads) {
      ...
      return !!(!this.textComposer.textIsEmpty || hasSendableAttachment || this.pollId || this.locationComposer.validLocation);
    }
...
var Channel = class {
   ...
   */
  async _sendMessage(rawMessage, options) {
    const client = this.getClient();
    const message = sanitizeOutgoingAttachments({
      client,
      message: rawMessage
    });
    return await client.post(this._channelURL() + "/message", {
      message,
      ...options
    });
  }
   ...
   */
  async queryMembers(filterConditions, sort = [], options = {}, requestOptions = {}) {
    let id;
  ...
  constructor({ client }) {
    ...
    this._req = async (params, config, retry) => {
      if (!this.cancelToken && !params.close) {
        this.cancelToken = import_axios3.default.CancelToken.source();
      }
      try {
...
var StreamChat = class _StreamChat {
  constructor(key, secretOrOptions, options) {
    ...
    this.doAxiosRequest = async (type, url, data, options = {}) => {
      ...
      try {
        ...
        if (import_axios4.default.isCancel(e)) throw e;
    ...
    this.axiosInstance = import_axios4.default.create(this.options);
  ...
  get(url, params, config) {
    return this.doAxiosRequest("get", url, null, { params, config });
  }
  put(url, data) {
    return this.doAxiosRequest("put", url, data);
  }
  post(url, data, config) {
    return this.doAxiosRequest("post", url, data, { config });
  }
  patch(url, data) {
   ...
   */
  async queryUsers(filterConditions, sort = [], options = {}, requestOptions = {}) {
    const defaultOptions = {
      presence: false
    };
   ...
   */
  async searchUserGroups(options, requestOptions = {}) {
    return await this.get(
   ...
   */
  async queryChannelsRequestWithResponse(filterConditions, sort = [], options = {}, requestOptions = {}) {
    const defaultOptions = {
      state: true,
      watch: true,
      presence: false
   ...
   * @return {Promise<SearchAPIResponse>} search messages response
   */
  async search(filterConditions, query, options = {}, requestOptions = {}) {
    if (options.offset && options.next) {
      throw Error(`Cannot specify offset with next`);
    }
    const payload = {
  ...
  _enrichAxiosOptions(options = {
    ...
    const resolvedSignal = options.config?.signal ?? axiosRequestConfigRest.signal ?? signal;
    return {
      params: {
        user_id: this.userID,
   ...
   * @returns {Promise<APIResponse>}
   */
  searchRoles(options, requestOptions = {}) {
    return this.get(
      ...
      try {
        ...
        if (lastSyncedAt) {
          ...
          if (diff > 30) {
            ...
            const exceedsLimit = typeof syncMaxEventCount === "number" && syncMaxEventCount > 0 && result.events.length > syncMaxEventCount;
            if (exceedsLimit) {
              this.client.logger(
                "warn",
        ...
        if ((0, import_axios5.isAxiosError)(e) && e.code === "ECONNABORTED") {
          return;
        }
        const error = e;
