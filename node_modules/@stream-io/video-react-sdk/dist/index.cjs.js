...
const Audio = react.memo(function AudioRender({ participant, trackType = 'audioTrack', ...rest }) {
...
function useDragToScroll(element, options = {}) {
    const { enabled, decay = 0.95, minVelocity = 0.5, dragThreshold = 5, } = options;
...
const attachHiddenMediaSink = (stream) => {
    const logger = videoClient.videoLoggerSystem.getLogger('useLoopbackRecording');
...
const useLoopbackRecording = () => {
    const call = videoReactBindings.useCall();
    const { useCallCallingState, useParticipantCount, useLocalParticipant } = videoReactBindings.useCallStateHooks();
    const callingState = useCallCallingState();
    const participantCount = useParticipantCount();
    const localParticipant = useLocalParticipant();
    const [recordingState, setRecordingState] = react.useState('idle');
    const recordingStateRef = react.useRef('idle');
    const isMountedRef = react.useRef(true);
    const awaitAbortRef = react.useRef(null);
    const stopRef = react.useRef(null);
    const runRef = react.useRef(null);
    const loopbackStreams = react.useMemo(() => videoClient.getLoopbackStreams(call, localParticipant), [call, localParticipant]);
    const updateState = react.useCallback((next) => {
    ...
    const startRecording = react.useCallback(async ({ includeVideo = true, maxDurationMs = videoClient.DEFAULT_LOOPBACK_RECORDING_DURATION_MS, } = {}) => {
        const logger = videoClient.videoLoggerSystem.getLogger('useLoopbackRecording');
        if (!call)
            return null;
        ...
        const run = async () => {
            ...
            try {
                const tracks = await videoClient.waitForLoopbackStreams(call, {
                    includeVideo,
                    signal: awaitAbortRef.current.signal,
                    timeoutMs: videoClient.LOOPBACK_STREAMS_WAIT_TIMEOUT_MS,
                });
                if (tracks === null) {
                ...
                const restoreAudio = videoClient.enableLoopbackAudio(tracks);
                const stream = new MediaStream([tracks.audioTrack, tracks.videoTrack].filter((track) => !!track));
                const detachSink = attachHiddenMediaSink(stream);
                const recorder = new videoClient.MediaStreamRecorder(stream, {
                    maxDurationMs: videoClient.clampLoopbackRecordingDuration(maxDurationMs),
                });
    ...
    react.useEffect(() => {
        if (callingState === videoClient.CallingState.LEFT ||
            callingState === videoClient.CallingState.IDLE) {
            stopRecording().catch(() => { });
        }
 ...
 */
const BaseVideo = react.forwardRef(function BaseVideoRender({ stream, ...rest }, ref) {
...
const DefaultVideoPlaceholder = react.forwardRef(function DefaultVideoPlaceholderRender(props, ref) {
    const { t } = videoReactBindings.useI18n();
    return (jsxRuntime.jsx(BaseVideoPlaceholder, { ref: ref, ...props, children: t('Video is disabled') }));
});

const DefaultPictureInPicturePlaceholder = react.forwardRef(function DefaultPictureInPicturePlaceholderRender(props, ref) {
    const { t } = videoReactBindings.useI18n();
    return (jsxRuntime.jsx(BaseVideoPlaceholder, { ref: ref, ...props, children: t('Video is playing in a popup') }));
});

const ABSOLUTE_POSITION_STYLE = { position: 'absolute' };
const Video$1 = ({ enabled = true, mirror, trackType, participant, className, VideoPlaceholder = DefaultVideoPlaceholder, PictureInPicturePlaceholder = DefaultPictureInPicturePlaceholder, refs, ...rest }) => {
    ...
    return (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [!hasNoVideoOrInvisible && (jsxRuntime.jsx("video", { ...rest, className: clsx('str-video__video', className, {
                ...
                } })), isPiP && PictureInPicturePlaceholder && (jsxRuntime.jsx(PictureInPicturePlaceholder, { style: ABSOLUTE_POSITION_STYLE, participant: participant })), (hasNoVideoOrInvisible || isVideoPaused) && VideoPlaceholder && (jsxRuntime.jsx(VideoPlaceholder, { style: ABSOLUTE_POSITION_STYLE, participant: participant, ref: refs?.setVideoPlaceholderElement }))] }));
};
Video$1.displayName = 'Video';

const useTrackElementVisibility = ({ trackedElement, sessionId, trackType, }) => {
    const call = videoReactBindings.useCall();
    react.useEffect(() => {
        if (!trackedElement || !call || trackType === 'none')
            return;
...
const BackgroundFiltersProviderImpl = react.lazy(() => Promise.resolve().then(function () { return require('./background-filters-DMStbSEg.cjs.js'); }).then((m) => ({
...
const IconButton = react.forwardRef(function IconButtonRender(props, ref) {
...
const CompositeButton = react.forwardRef(function CompositeButtonRender({ disabled, caption, children, className, active, Menu, menuPlacement, menuOffset, title, ToggleMenuButton = DefaultToggleMenuButton, variant, onClick, onMenuToggle, ...restButtonProps }, ref) {
...
const DefaultToggleMenuButton = react.forwardRef(function DefaultToggleMenuButtonRender({ menuShown }, ref) {
...
const ToggleEndRecordingMenuButton = react.forwardRef(function ToggleEndRecordingMenuButtonRender(props, ref) {
...
const ToggleReactionsMenuButton = react.forwardRef(function ToggleReactionsMenuButtonRender({ menuShown }, ref) {
    const { t } = videoReactBindings.useI18n();
...
const ScreenShareButton = (props) => {
    ...
    const { hasPermission, requestPermission, isAwaitingPermission } = useRequestPermission(videoClient.RequestPermissionRequestPermissionsEnum.SCREENSHARE);
...
const AudioVolumeIndicator = () => {
    ...
    react.useEffect(() => {
        if (!isEnabled || !mediaStream)
            return;
        const disposeSoundDetector = videoClient.createSoundDetector(mediaStream, ({ audioLevel: al }) => {
            if (barRef.current) {
    ...
    return (jsxRuntime.jsxs("div", { className: "str-video__audio-volume-indicator", children: [jsxRuntime.jsx(Icon, { icon: isEnabled ? 'mic' : 'mic-off' }), jsxRuntime.jsx("div", { className: "str-video__audio-volume-indicator__bar", children: jsxRuntime.jsx("div", { ref: barRef, className: "str-video__audio-volume-indicator__bar-value" }) })] }));
};
...
const ToggleDeviceSettingsMenuButton = react.forwardRef(function ToggleDeviceSettingsMenuButtonRender({ menuShown }, ref) {
    const { t } = videoReactBindings.useI18n();
...
const ToggleAudioPreviewButton = (props) => {
    ...
    const { t } = videoReactBindings.useI18n();
    const { useMicrophoneState, useLocalParticipant } = videoReactBindings.useCallStateHooks();
    ...
    const localParticipant = useLocalParticipant();
    const isSystemMuted = !!localParticipant?.interruptedTracks?.includes(videoClient.SfuModels.TrackType.AUDIO);
    ...
    return (jsxRuntime.jsx(WithTooltip, { title: !hasBrowserPermission
            ...
            : isSystemMuted
                ...
                onMenuToggle?.(shown);
            }, children: [jsxRuntime.jsx(Icon, { icon: !optionsAwareIsMute ? 'mic' : 'mic-off' }), !hasBrowserPermission && (jsxRuntime.jsx("span", { className: "str-video__no-media-permission", title: t('Check your browser audio permissions'), children: "!" })), isPromptingPermission && (jsxRuntime.jsx("span", { className: "str-video__pending-permission", title: t('Waiting for permission'), children: "?" })), isSystemMuted && hasBrowserPermission && (jsxRuntime.jsx("span", { className: "str-video__system-muted", title: t('Microphone is paused by your system'), children: "!" }))] }) }));
};
const ToggleAudioPublishingButton = (props) => {
    ...
    const { hasPermission, requestPermission, isAwaitingPermission } = useRequestPermission(videoClient.RequestPermissionRequestPermissionsEnum.SEND_AUDIO);
    const { useMicrophoneState, useLocalParticipant } = videoReactBindings.useCallStateHooks();
    ...
    return (jsxRuntime.jsx(videoReactBindings.Restricted, { requiredGrants: [videoClient.OwnCapability.SEND_AUDIO], children: jsxRuntime.jsx(PermissionNotification, { permission: videoClient.OwnCapability.SEND_AUDIO, isAwaitingApproval: isAwaitingPermission, messageApproved: t('You can now speak.'), messageAwaitingApproval: t('Awaiting for an approval to speak.'), messageRevoked: t('You can no longer speak.'), children: jsxRuntime.jsx(WithTooltip, { title: !hasPermission
                    ...
                    : !hasBrowserPermission
                        ...
                        setTooltipDisabled(shown);
                        onMenuToggle?.(shown);
                    }, children: [jsxRuntime.jsx(Icon, { icon: optionsAwareIsMute ? 'mic-off' : 'mic' }), (!hasBrowserPermission || !hasPermission) && (jsxRuntime.jsx("span", { className: "str-video__no-media-permission", children: "!" })), isPromptingPermission && (jsxRuntime.jsx("span", { className: "str-video__pending-permission", title: t('Waiting for permission'), children: "?" })), isSystemMuted && hasBrowserPermission && hasPermission && (jsxRuntime.jsx("span", { className: "str-video__system-muted", title: t('Microphone is paused by your system'), children: "!" }))] }) }) }) }));
};
...
const ToggleVideoPreviewButton = (props) => {
    ...
    const { t } = videoReactBindings.useI18n();
    const { useCameraState, useLocalParticipant } = videoReactBindings.useCallStateHooks();
    ...
    const localParticipant = useLocalParticipant();
    const isSystemMuted = !!localParticipant?.interruptedTracks?.includes(videoClient.SfuModels.TrackType.VIDEO);
    const [tooltipDisabled, setTooltipDisabled] = react.useState(false);
    ...
    return (jsxRuntime.jsx(WithTooltip, { title: !hasBrowserPermission
            ...
            : isSystemMuted
                ...
                setTooltipDisabled(shown);
                onMenuToggle?.(shown);
            }, children: [jsxRuntime.jsx(Icon, { icon: !optionsAwareIsMute ? 'camera' : 'camera-off' }), !hasBrowserPermission && (jsxRuntime.jsx("span", { className: "str-video__no-media-permission", title: t('Check your browser video permissions'), children: "!" })), isPromptingPermission && (jsxRuntime.jsx("span", { className: "str-video__pending-permission", title: t('Waiting for permission'), children: "?" })), isSystemMuted && hasBrowserPermission && (jsxRuntime.jsx("span", { className: "str-video__system-muted", title: t('Camera is paused by your system'), children: "!" }))] }) }));
};
const ToggleVideoPublishingButton = (props) => {
    ...
    const { hasPermission, requestPermission, isAwaitingPermission } = useRequestPermission(videoClient.RequestPermissionRequestPermissionsEnum.SEND_VIDEO);
    const { useCameraState, useCallSettings, useLocalParticipant } = videoReactBindings.useCallStateHooks();
    ...
    return (jsxRuntime.jsx(videoReactBindings.Restricted, { requiredGrants: [videoClient.OwnCapability.SEND_VIDEO], children: jsxRuntime.jsx(PermissionNotification, { permission: videoClient.OwnCapability.SEND_VIDEO, isAwaitingApproval: isAwaitingPermission, messageApproved: t('You can now share your video.'), messageAwaitingApproval: t('Awaiting for an approval to share your video.'), messageRevoked: t('You can no longer share your video.'), children: jsxRuntime.jsx(WithTooltip, { title: !hasPermission
                    ...
                    : !hasBrowserPermission
                        ...
                        onMenuToggle?.(shown);
                            ...
                            !hasPermission ||
                            !isPublishingVideoAllowed) && (jsxRuntime.jsx("span", { className: "str-video__no-media-permission", children: "!" })), isPromptingPermission && (jsxRuntime.jsx("span", { className: "str-video__pending-permission", title: t('Waiting for permission'), children: "?" })), isSystemMuted &&
                            hasBrowserPermission &&
                            hasPermission &&
                            isPublishingVideoAllowed && (jsxRuntime.jsx("span", { className: "str-video__system-muted", title: t('Camera is paused by your system'), children: "!" }))] }) }) }) }));
};
...
const CancelCallToggleMenuButton = react.forwardRef(function CancelCallToggleMenuButtonRender({ menuShown }, ref) {
    const { t } = videoReactBindings.useI18n();
...
const ToggleMenuButton = react.forwardRef(function ToggleMenuButtonRender(props, ref) {
    const { t } = videoReactBindings.useI18n();
...
const CallParticipantListingItem = react.memo(function CallParticipantListingItemRender({ participant, DisplayName = DefaultDisplayName, }) {
...
const ToggleButton$2 = react.forwardRef(function ToggleButtonRender(props, ref) {
...
const SearchInput = ({ exitSearch, isActive, ...rest }) => {
    ...
    const onExitSearch = videoReactBindings.useEffectEvent(exitSearch);
    react.useEffect(() => {
        if (!inputElement)
            return;
...
const BlockedUsersSearchResults = ({ blockedUsersSearchFn: blockedUsersSearchFnFromProps, debounceSearchInterval, searchQuery, }) => {
    ...
    const blockedUsersSearchFn = react.useCallback(async (queryString) => {
        ...
        return blockedUsers.filter((userId) => userId.toLowerCase().includes(normalizedQuery));
    }, [blockedUsers]);
    const { searchQueryInProgress, searchResults } = useSearch({
...
const ToggleButton$1 = react.forwardRef(function ToggleButtonRender(props, ref) {
...
const PermissionRequestList = react.forwardRef(function PermissionRequestListRender(props, ref) {
...
const ToggleButton = react.forwardRef(function ToggleButtonRender(props, ref) {
    return jsxRuntime.jsx(IconButton, { enabled: props.menuShown, icon: "ellipsis", ref: ref });
});
...
const DefaultAudioBlockedNotification = () => {
    const call = videoReactBindings.useCall();
    const { t } = videoReactBindings.useI18n();
    return (jsxRuntime.jsx("button", { type: "button", className: "str-video__audio-blocked-notification", onClick: call?.resumeAudio, "aria-live": "polite", children: jsxRuntime.jsxs("span", { className: "str-video__audio-blocked-notification__message", children: [jsxRuntime.jsx(Icon, { icon: "speaker" }), jsxRuntime.jsxs("span", { className: "str-video__audio-blocked-notification__text", children: [jsxRuntime.jsx("span", { className: "str-video__audio-blocked-notification__title", children: t('Audio is blocked by your system') }), jsxRuntime.jsx("span", { className: "str-video__audio-blocked-notification__subtitle", children: t('Click or tap to play audio') })] })] }) }));
};
const DefaultParticipantViewUI = ({ indicatorsVisible = true, menuPlacement = 'bottom-start', showMenuButton = true, ParticipantActionsContextMenu: ParticipantActionsContextMenu$1 = ParticipantActionsContextMenu, AudioBlockedNotification = DefaultAudioBlockedNotification, }) => {
    ...
    const { useAutoplayBlockedSessionIds } = videoReactBindings.useCallStateHooks();
    ...
    return (jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [showMenuButton && (jsxRuntime.jsx(MenuToggle, { strategy: "fixed", placement: menuPlacement, ToggleButton: ToggleButton, children: jsxRuntime.jsx(ParticipantActionsContextMenu$1, {}) })), jsxRuntime.jsx(Reaction, { participant: participant }), jsxRuntime.jsx(ParticipantDetails, { indicatorsVisible: indicatorsVisible }), AudioBlockedNotification && shouldShowAudioBlockedNotification && (jsxRuntime.jsx(AudioBlockedNotification, {}))] }));
};
...
const ParticipantView = react.memo(react.forwardRef(function ParticipantViewRender({ participant, trackType = 'videoTrack', mirror, muteAudio, refs: { setVideoElement, setVideoPlaceholderElement } = {}, className, VideoPlaceholder, PictureInPicturePlaceholder, ParticipantViewUI = DefaultParticipantViewUI, }, ref) {
...
var en = {
	...
	"Microphone is paused by your system": "Microphone is paused by your system",
	"Camera on": "Camera on",
	"Camera off": "Camera off",
	"Camera is paused by your system": "Camera is paused by your system",
	...
	"Video is connecting...": "Video is connecting...",
...
const BackstageLayout = (props) => {
    ...
    return (jsxRuntime.jsx("div", { className: "str-video__livestream-layout__wrapper", children: jsxRuntime.jsxs("div", { className: "str-video__livestream-layout__backstage", children: [startsAt && (jsxRuntime.jsx("span", { className: "str-video__livestream-layout__starts-at", children: startsAtPassed
...
const PaginatedGridLayoutGroup = ({ group, mirror, VideoPlaceholder, PictureInPicturePlaceholder, ParticipantViewUI, }) => {
    return (jsxRuntime.jsx("div", { "data-group-size": group.length, className: clsx('str-video__paginated-grid-layout__group', {
...
const SpeakerLayout = ({ ParticipantViewUIBar = DefaultParticipantViewUIBar, ParticipantViewUISpotlight = DefaultParticipantViewUI, VideoPlaceholder, PictureInPicturePlaceholder, participantsBarPosition = 'bottom', participantsBarLimit, mirrorLocalParticipantVideo = true, excludeLocalParticipant = false, filterParticipants, pageArrowsVisible = true, muted, enableDragToScroll = false, }) => {
    ...
    videoReactBindings.useSpeakerLayoutSortPreset(call, isOneOnOneCall);
...
const [major, minor, patch] = ("1.43.1").split('.');
...
exports.DefaultAudioBlockedNotification = DefaultAudioBlockedNotification;
